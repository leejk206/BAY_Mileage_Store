...
- [x] **CAT-01**: 운영자가 상품을 온체인에 등록할 수 있다 (이름, 가격(BAY), 재고)
- [x] **CAT-02**: 상품 목록과 가격을 온체인에서 읽을 수 있다

### Purchase

- [x] **PUR-01**: 학회원이 특정 상품을 선택해 BAY 토큰을 소각(burn)하는 구매 트랜잭션을 전송할 수 있다
- [x] **PUR-02**: 구매 성공 시 온체인에 구매 기록(PDA)이 생성된다 (구매자, 상품, 타임스탬프)
- [x] **PUR-03**: 잔액 부족 시 구매 트랜잭션이 실패한다
...
| PUR-01 | Phase 2 | Complete |
| PUR-02 | Phase 2 | Complete |
| PUR-03 | Phase 2 | Complete |
